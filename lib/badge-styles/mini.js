...
export function render (pkginfo, options, params) {
  ...
  const strokeWidth = 1
  const bgRect = createRect(strokeWidth / 2, strokeWidth / 2, width - strokeWidth, height - strokeWidth, 3, 3, 'rgb(244, 244, 242)', 'rgb(203, 56, 55)', strokeWidth)

  // NPM logo (tiny for mini)
  const logo = `
    <!-- Mini version npm logo -->
${createNpmLogo(margin + 3.5, margin + 3.5, 0.06)}`
