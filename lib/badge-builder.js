...
export function createRect (x, y, width, height, rx, ry, fill, stroke = null, strokeWidth = 0) {
  const strokeAttrs = stroke ? `stroke="${escapeXml(stroke)}" stroke-width="${strokeWidth}"` : ''
  return `  <rect x="${x}" y="${y}"
...
export function createNpmLogo (x, y, scale, fillColor = 'rgb(203, 56, 55)', bgColor = 'rgb(244, 244, 242)') {
  ...
  paths += `        <path fill="${escapeXml(fillColor)}" d="${NPM_LOGO_PATH}"/>\n`
  NPM_LOGO_PATHS.forEach(path => {
    paths += `<path fill="${escapeXml(bgColor)}" d="${path}"/>\n`
  })
  paths += '      </g>\n'
  paths += '    </g>'
  return paths
