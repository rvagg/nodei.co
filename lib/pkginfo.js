...
function loadDepends (pkg, callback) {
  ...
  hyperquest(url).pipe(bl(function (err, body) {
    if (err)
      return callback(err)
    try {
...
function loadDoc (pkg, callback) {
  hyperquest(registryUrl + pkg).pipe(bl(function (err, body) {
    if (err)
      return callback(err)
    try {
      var doc = JSON.parse(body.slice().toString())
      ...
      if (!(version = doc['dist-tags'].latest))
         return callback(new Error('no dit-tags.latest found'))
      ...
      callback(null, {
        ...
        , dependencies : doc.versions && doc.versions[version]
            && doc.versions[version].dependencies
