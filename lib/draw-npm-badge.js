...
function downloadRankDescriptor (ranking) {
  ...
  var pr = (ranking.rank / ranking.total) * 100
    , i
...
function calculateParams (options, pkginfo) {
  var margin        = options.mini ? 1 : MARGIN
    ...
    , compactText   = options.compact
                        && ('v' + (!!pkginfo.version ? pkginfo.version : '') + ' ' + updatedText)
    ...
    , showDownloadRank = options.downloadRank && !!pkginfo.ranking
    , totalPackages = showDownloadRank ? Math.round(pkginfo.ranking.total / 500) * 500 : 1
    , downloadRankText = showDownloadRank
                        ? 'download rank: '
                          + downloadRankDescriptor(pkginfo.ranking)
                          + ' of '
                          + humanize(totalPackages)
                          + ' packages'
                        : ''
